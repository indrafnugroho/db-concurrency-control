...
// Modified by: Jovankaruna (13518024@std.stei.itb.ac.id)
...
string ModeToString(CCMode mode) {
  switch (mode) {
    case LOCKING_EXCLUSIVE_ONLY: return " Simple Lock ";
    case OCC:                    return " OCC ";
...
void Benchmark(const vector<LoadGen*>& lg, CCMode mode) {
...
int main(int argc, char** argv) {
  ...
  vector<LoadGen*> lg;
  //Print mode name
  for (CCMode mode = LOCKING_EXCLUSIVE_ONLY ; mode <= MVCC; mode = static_cast<CCMode>(mode+1)) {
    cout << "Average" << ModeToString(mode) << "Transaction Duration" << endl;
